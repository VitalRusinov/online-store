import mirror_01 from '../../assets/pictures/mirrors/mirror_01.png';
import mirror_02 from '../../assets/pictures/mirrors/mirror_02.png';
import mirror_03 from '../../assets/pictures/mirrors/mirror_03.png';
import mirror_04 from '../../assets/pictures/mirrors/mirror_04.png';
import mirror_05 from '../../assets/pictures/mirrors/mirror_05.png';
import mirror_06 from '../../assets/pictures/mirrors/mirror_06.png';
import mirror_07 from '../../assets/pictures/mirrors/mirror_07.png';
import mirror_08 from '../../assets/pictures/mirrors/mirror_08.png';
import mirror_09 from '../../assets/pictures/mirrors/mirror_09.png';
import mirror_10 from '../../assets/pictures/mirrors/mirror_10.png';

export const mirrorsInitialState = {
  entries: [
    {
      id: 'mirror_01',
      image: mirror_01,
      product: 'Зеркало напольное',
      name: 'Lunaris',
      article: 'MIR001',
      color: 'lavender',
      description: 'Зеркало с лунными узорами, излучающее мягкое ночное сияние',
      designer: 'Elara Moonshadow',
      size: {
        height: '160см',
      },
      price: 150000,
    },
    {
      id: 'mirror_02',
      image: mirror_02,
      product: 'Зеркало напольное',
      name: 'Glade',
      article: 'MIR002',
      color: 'green',
      description:
        'Зеркало, окруженное ветвями и кристаллами, напоминающими лесную поляну',
      designer: 'Thalion Elensar',
      size: {
        height: '160см',
      },
      price: 155000,
    },
    {
      id: 'mirror_03',
      image: mirror_03,
      product: 'Зеркало настенное',
      name: 'Starlight',
      article: 'MIR003',
      color: 'lazure',
      description:
        'Зеркало, украшенное звёздными мотивами, отражающее небесное мерцание',
      designer: 'Lirael Windwalker',
      size: {
        height: '120см',
      },
      price: 130000,
    },
    {
      id: 'mirror_04',
      image: mirror_04,
      product: 'Зеркало настенное',
      name: 'Sylva',
      article: 'MIR004',
      color: 'green',
      description: 'Изящное зеркало с рамкой в виде эльфийских лесных узоров',
      designer: 'Elysia Greenleaf',
      size: {
        height: '130см',
      },
      price: 140000,
    },
    {
      id: 'mirror_05',
      image: mirror_05,
      product: 'Зеркало напольное',
      name: 'Echo',
      article: 'MIR005',
      color: 'orange',
      description:
        'Зеркало с рамкой из переплетающихся ветвей, создающее лесной эффект',
      designer: 'Caelith Brightblade',
      size: {
        height: '160см',
      },
      price: 145000,
    },
    {
      id: 'mirror_06',
      image: mirror_06,
      product: 'Зеркало настенное',
      name: 'Twilight',
      article: 'MIR006',
      color: 'blue',
      description: 'Зеркало с гранями, мерцающими в сумеречном свете',
      designer: 'Arwen Nightshade',
      size: {
        height: '120см',
      },
      price: 135000,
    },
    {
      id: 'mirror_07',
      image: mirror_07,
      product: 'Зеркало настенное',
      name: 'Whisper',
      article: 'MIR007',
      color: 'lazure',
      description:
        'Зеркало, украшенное узорами, напоминающими шёпот лесных духов',
      designer: 'Nimrodel Leafsong',
      size: {
        height: '130см',
      },
      price: 142000,
    },
    {
      id: 'mirror_08',
      image: mirror_08,
      product: 'Зеркало настенное',
      name: 'Elvara',
      article: 'MIR008',
      color: 'pink',
      description:
        'Зеркало с рамкой, отражающей эльфийскую утонченность и мастерство',
      designer: 'Ayla Silverleaf',
      size: {
        height: '120см',
      },
      price: 150000,
    },
    {
      id: 'mirror_09',
      image: mirror_09,
      product: 'Зеркало напольное',
      name: 'Mystique',
      article: 'MIR009',
      color: 'blue',
      description:
        'Зеркало с таинственной рамкой, вдохновлённой мифами и легендами',
      designer: 'Finarfin Starbreeze',
      size: {
        height: '160см',
      },
      price: 160000,
    },
    {
      id: 'mirror_10',
      image: mirror_10,
      product: 'Зеркало настенное',
      name: 'Fern',
      article: 'MIR010',
      color: 'green',
      description: 'Зеркало с рамкой, украшенной узорами листьев папоротника',
      designer: 'Lucas Fernwood',
      size: {
        height: '130см',
      },
      price: 140000,
    },
  ],
};
